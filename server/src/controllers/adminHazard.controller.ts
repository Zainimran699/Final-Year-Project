...
export async function listQuestions(_req: Request, res: Response) {
  try {
    ...
    console.error("adminHazard listQuestions error:", err);
...
export async function createQuestion(req: Request, res: Response) {
  try {
    ...
    console.error("adminHazard createQuestion error:", err);
    return res.status(500).json({ error: "Internal server error" });
  }
}
...
export async function updateQuestion(req: Request, res: Response) {
  try {
    ...
    console.error("adminHazard updateQuestion error:", err);
    return res.status(500).json({ error: "Internal server error" });
  }
}
...
export async function deleteQuestion(req: Request, res: Response) {
  try {
    ...
    console.error("adminHazard deleteQuestion error:", err);
    return res.status(500).json({ error: "Internal server error" });
  }
}
